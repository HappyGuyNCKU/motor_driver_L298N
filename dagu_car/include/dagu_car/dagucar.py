...
from motor_control.DCmotor_HAT import DCmotor_HAT , DCMotor
import time
import atexit
import numpy
import warnings
...
class DAGU_Car:
    ...
    def __init__(self, verbose=False):
        ...
        self.mh = DCmotor_HAT(addr=0x60)
        self.rearMotor = self.mh.getMotor(1)
        self.frontMotor = self.mh.getMotor(2)
        self.rearMotorMode = DCmotor_HAT.RELEASE
    ...
    def setSpeed(self, speed):
        ...
        if abs(speed) < 1e-4:
            ...
            self.rearMotorMode = DCmotor_HAT.FORWARD
        elif speed < 0: 
            self.rearMotorMode = DCmotor_HAT.BACKWARD
    ...
    def steerServo(self):
        while not self.steerThreadStop:
            ...
            steerMotorMode = DCmotor_HAT.RELEASE
            ...
            if(abs(delta)>DAGU_Car.__steerTolerance):
                if(cmd>0):
                    steerMotorMode = DCmotor_HAT.BACKWARD
                else:
                    steerMotorMode = DCmotor_HAT.FORWARD
                cmd = abs(cmd)
    ...
    def stopSteerControl(self):
        ...
        self.frontMotor.run(DCmotor_HAT.RELEASE)
    ...
    def turnOffMotors(self):
        self.rearMotor.run(DCmotor_HAT.RELEASE)
        self.frontMotor.run(DCmotor_HAT.RELEASE)
