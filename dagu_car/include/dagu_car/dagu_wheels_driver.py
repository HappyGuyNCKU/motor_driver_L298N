...
class DaguWheelsDriver:
    ...
    def __init__(self, verbose=False, debug=False, left_flip=False, right_flip=False):
        self.motorhat = DCmotor_HAT(addr=0x60)
    ...
    def updatePWM(self):
        ...
        if fabs(vl) < self.SPEED_TOLERANCE:
            leftMotorMode = DCmotor_HAT.RELEASE
        elif vl > 0:
            leftMotorMode = DCmotor_HAT.FORWARD
        elif vl < 0: 
            leftMotorMode = DCmotor_HAT.BACKWARD

        if fabs(vr) < self.SPEED_TOLERANCE:
            rightMotorMode = DCmotor_HAT.RELEASE
            pwmr = 0
        elif vr > 0:
            rightMotorMode = DCmotor_HAT.FORWARD
        elif vr < 0: 
            rightMotorMode = DCmotor_HAT.BACKWARD
    ...
    def __del__(self):
        self.leftMotor.run(DCmotor_HAT.RELEASE)
        self.rightMotor.run(DCmotor_HAT.RELEASE)
        del self.motorhat
